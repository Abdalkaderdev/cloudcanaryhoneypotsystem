...
export function Infrastructure({ items }: { items: { name: string; status: "ok" | "warn" | "down" }[] }) {
  return (
    <Frame kicker="No. V" title="On the watch" byline="The state of every component in the observation chain.">
      <ul className="space-y-0">
        {items.map((it) => {
          ...
          return (
            <li key={it.name} className="flex items-baseline justify-between py-3 border-b border-rule/60 last:border-0">
              <span className="font-display text-ink" style={{ fontSize: "17px", fontWeight: 500 }}>{it.name}</span>
